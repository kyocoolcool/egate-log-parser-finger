...
public class MrzParser {
    ...
    public boolean checkDigit(int col, int row, String str, String fieldName) {
        ...
        if (digit != checkDigit) {
            ...
            //logger.info("Check digit verification failed for " + fieldName + ": expected " + digit + " but got " + checkDigit);
        }
    ...
    public MrzDate parseDate(MrzRange range) {
        ...
        try {
            ...
            year = -1;
            //logger.debug("Failed to parse MRZ date year " + rawValue(range) + ": " + ex, mrz, r);
            
        }
        if (year < 0 || year > 99) {
            //logger.debug("Invalid year value " + year + ": must be 0..99");
        }
        ...
        try {
            ...
            month = -1;
            //logger.debug("Failed to parse MRZ date month " + rawValue(range) + ": " + ex, mrz, r);
        }
        if (month < 1 || month > 12) {
            //logger.debug("Invalid month value " + month + ": must be 1..12");
        ...
        if (day < 1 || day > 31) {
            //logger.debug("Invalid day value " + day + ": must be 1..31");
        }
