...
public class LogToExcel_R1T5 implements AutoCloseable {
...
//	private static final int[] styleBColumn = {1, 5, 7, 8, 11, 12, 13, 18, 19, 20, 24, 25, 26, 30, 31, 32};
	...
	private static final String[] columnName = new String[] { //與ColumnName0 欄位互相對應, ColumnName0 是合併ColumnName的區塊
			...
			"", //證件到期時間
			"不成功", "成功", //通關狀況
			"護照機啟動", "eGate送查驗系統", "秒數", //證件檢核時間A
			"eGate送查驗系統", "eGate收到查驗系統回覆", "秒數", "RCODE",//身分查驗時間B
			"開始時間", "結束時間", "秒數",
			"eGate收到查驗系統回覆", "第一道門開啟", "秒數", //第一道門開啟時間C
			"第一道門開啟","第一道門關閉","秒數", //旅客使用時間D
			"第一道門關閉", "第二道門開啟", "秒數", "臉辨分數", "指辨分數", //身份辨識時間E			
			"", //eGate使用時間\nA+C+E
			"" //整體通關時間\nA+B+C+D+E
			};
	...
	public LogToExcel_R1T5() {
		wb = new SXSSFWorkbook();
	}
	
	public LogToExcel_R1T5(SXSSFWorkbook wb) {
		this.wb = wb;
	}
...
//		sheet.addMergedRegion(new CellRangeAddress(rowIndex-1, rowIndex, 33, 33));
	...
	private Object[] beanValueToArray(PassengerLog u, int cnt) {
		...
		arr[idx++] = CommonUtils.decimalRoundD(u.getReportASeconds(), 3); //秒數
		...
		arr[idx++] = CommonUtils.DateTimeToTime(u.getQryHywebEndTime()); //eGate收到查驗系統回覆
		arr[idx++] = CommonUtils.DateTimeToTime(u.getDoor1aOpenTime()); //第一道門開啟
		arr[idx++] = CommonUtils.decimalRoundD(u.getReportCSeconds(), 3); //秒數
		arr[idx++] = CommonUtils.DateTimeToTime(u.getDoor1aOpenTime()); //第一道門開啟
		arr[idx++] = CommonUtils.DateTimeToTime(u.getDoor1aCloseTime()); //第一道門關閉
		arr[idx++] = CommonUtils.decimalRoundD(u.getReportDSeconds(), 3); //秒數
		arr[idx++] = CommonUtils.DateTimeToTime(u.getDoor1aCloseTime()); //第一道門關閉
		arr[idx++] = CommonUtils.DateTimeToTime(u.getDoor2aOpenTime()); //第二道門開啟
		arr[idx++] = CommonUtils.decimalRoundD(u.getReportESeconds(), 3); //秒數
		arr[idx++] = u.getFaceScore(); //臉辨分數
		arr[idx++] = u.getFingerScore(); //指辨分數
		arr[idx++] = CommonUtils.decimalRoundD(u.getReportACESeconds(), 3); //第三代自動通關系統所需時間\nA+C+D
		arr[idx++] = CommonUtils.decimalRoundD(u.getReportABCDESeconds(), 3); //整體通關時間\nA+B+C+D+E		
		return arr;
