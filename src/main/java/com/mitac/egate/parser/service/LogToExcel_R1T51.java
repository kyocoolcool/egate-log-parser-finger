...
public class LogToExcel_R1T51 implements AutoCloseable {
//	private static final int COLUMNS = 34;
	...
	public LogToExcel_R1T51() {
		wb = new SXSSFWorkbook();
	}
	
	public LogToExcel_R1T51(SXSSFWorkbook wb) {
		this.wb = wb;
	}
...
//		sheet.addMergedRegion(new CellRangeAddress(rowIndex, rowIndex, 30, 32));
	...
	private void writeMarkData() {
		String markData = "證件檢核時間(A), 身分查驗時間(B), 第一道門開啟時間(C), 旅客使用時間(D), 身份辨識時間(E)";
	...
	private Object[] beanValueToArray(PassengerAnalysis u) {
		...
		arr[idx++] = u.getAce_N_le_10();
		arr[idx++] = u.getAce_10_lt_N_le_12();
		arr[idx++] = u.getAce_12_lt_N_le_15();
		arr[idx++] = u.getAce_N_gt_15();
		arr[idx++] = u.getAbcde_N_le_10();
		arr[idx++] = u.getAbcde_10_lt_N_le_12();
		arr[idx++] = u.getAbcde_12_lt_N_le_15();
		arr[idx++] = u.getAbcde_N_gt_15();
