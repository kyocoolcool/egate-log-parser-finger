/**
   * 外籍(FOREIGN人士 每日通關EGATE使用時間大於10秒的明細表
   * 只保留成功紀錄
 */
...
public class LogToExcel_R2F5 implements AutoCloseable {
...
//	private static final int[] styleBColumn = {1, 5, 7, 8, 11, 12, 13, 18, 19, 20, 24, 25, 26, 30, 31, 32};
	...
	private static final String[] columnName = new String[] { //與ColumnName0 欄位互相對應, ColumnName0 是合併ColumnName的區塊
			...
			"秒數", //指紋比對時間
			"秒數", //第二道門開啟時間(F)
			"A>10", "A+C>10", "A+E11+E12+E21+E22+F>10","原因"
			};
	...
	public LogToExcel_R2F5() {
		wb = new SXSSFWorkbook();
	}
	
	public LogToExcel_R2F5(SXSSFWorkbook wb) {
		this.wb = wb;
	}
...
//		sheet.addMergedRegion(new CellRangeAddress(rowIndex-1, rowIndex, 33, 33));
	...
	private Object[] beanValueToArray(PassengerLog u, int cnt) {
		Object[] arr = new Object[columnName0.length];
		int idx = 0;
		arr[idx++] = u.geteGateId();
		arr[idx++] = u.getLogDate();
		arr[idx++] = String.valueOf(cnt);
		arr[idx++] = u.getMainDocument(); //證件號碼
		arr[idx++] = u.getNationality(); //國籍
		arr[idx++] = u.getEmrpExpireDate();
		arr[idx++] = CommonUtils.decimalRoundD(u.getReportACE1E2FSeconds(), 3); //第三代自動通關系統所需時間 A + C + E1 + E22 + F	
		...
		arr[idx++] = CommonUtils.decimalRoundD(u.getFaceNoResultSecs(), 3); //臉辨時間-秒數E11
		arr[idx++] = CommonUtils.decimalRoundD(u.getFaceHasResultSecs(), 3); //臉辨時間-秒數E12		
		arr[idx++] = CommonUtils.decimalRoundD(u.getFingerPressNoResultSecs(), 3); //按壓指紋時間-秒數E21
		arr[idx++] = CommonUtils.decimalRoundD(u.getFingerPressHasResultSecs(), 3); //按壓指紋時間-秒數E22
		arr[idx++] = CommonUtils.decimalRoundD(u.getReportFSeconds(), 3); //"第二道門開啟時間(F)
		arr[idx++] = u.getReportASeconds() > 10.0 ? CommonUtils.decimalRoundD(u.getReportASeconds(), 3) : null; //第三代自動通關系統所需時間 A>10
		/** 一定要用CommonUtils.calculateSecond()計算秒數，可避開null的狀況 **/
		...
		arr[idx++] = CommonUtils.calculateSecond(u.getReportASeconds(), u.getFaceNoResultSecs(), u.getFaceHasResultSecs(), u.getFingerPressNoResultSecs(), u.getFingerPressHasResultSecs(), u.getReportFSeconds()) > 10.0 ? 
				CommonUtils.decimalRoundD(CommonUtils.calculateSecond(u.getReportASeconds(), u.getFaceNoResultSecs(), u.getFaceHasResultSecs(), u.getFingerPressNoResultSecs(), u.getFingerPressHasResultSecs(), u.getReportFSeconds()), 3) : null; //第三代自動通關系統所需時間 A + E11 + E21 + E21 + E22 + F >10
		arr[idx++] = "";
