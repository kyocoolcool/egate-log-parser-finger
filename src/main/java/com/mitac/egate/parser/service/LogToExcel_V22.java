...
public class LogToExcel_V22 implements AutoCloseable {
...
//	private static final int[] styleBColumn = {1, 5, 7, 8, 11, 12, 13, 18, 19, 20, 24, 25, 26, 30, 31, 32};
	...
	public LogToExcel_V22() {
		wb = new SXSSFWorkbook();
	}
	
	public LogToExcel_V22(SXSSFWorkbook wb) {
		this.wb = wb;
	}
	...
	private void writeData() {
		...
		list.stream()
			...
			.forEach(u -> {
				...
				if (!((String)beanValue[2]).equals(xxx.get())) {
					if (ooo.get() != rowIndex) {
						sheet.addMergedRegion(new CellRangeAddress(ooo.get()-1, rowIndex-1, 2, 2));
					}
					ooo.set(rowIndex+1);
					xxx.set((String)beanValue[2]);
				}
				boolean flag = false;
